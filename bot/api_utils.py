...
class TelegramQuestion:
    ...
    def choices_letters(self):
        return [
            f'{choice["content"].split(":")[0].strip("*")}' for choice in self.choices
        ]

    @property
    def content(self):
        return (
            f'{QUESTION_MARK} {self.question["content"]}'
    ...
    def choices_str(self):
        choices_str = "\n".join(f'- {choice["content"]}' for choice in self.choices)
...
class TelegramAnswer:
    ...
    def content(self):
        return (
            f'{QUESTION_MARK} {self.answer["content"]}'
    ...
    def selected_choice_str(self, selected_choice_id):
        ...
        answer_str = (
            CORRECT_CHOICE_STR if self.is_correct is True else INCORRECT_CHOICE_STR
        )
